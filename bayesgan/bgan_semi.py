    ...
    def build_bgan_graph(self):
        ...
        for di, disc_params in enumerate(self.disc_param_list):
            ...
            for gi, gen_params in enumerate(self.gen_param_list):
                a = self.generator(self.z[:, :, gi % self.num_gen], gen_params)
                d_probs_, d_logits_, _ = self.discriminator(a,self.K, disc_params)
            ...
            d_loss_sup = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=d_sup_logits,
                                                                                labels=self.labels))
            d_losses_semi = []
